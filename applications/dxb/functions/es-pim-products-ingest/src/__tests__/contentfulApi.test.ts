...
jest.mock("@bmi-digital/functions-secret-client", () => ({
  getSecret: (secretName: string): Promise<string> => getSecret(secretName)
}));
...
describe("contentfulApi", () => {
  ...
  describe("getAssetTypes", () => {
    ...
    it("sholud return undefined if MANAGEMENT_ACCESS_TOKEN_SECRET is NOT provided", async () => {
      const token = process.env.MANAGEMENT_ACCESS_TOKEN_SECRET;
      delete process.env.MANAGEMENT_ACCESS_TOKEN_SECRET;

      console.log(process.env.MANAGEMENT_ACCESS_TOKEN_SECRET);
