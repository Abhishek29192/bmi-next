...
exports.pluginOptionsSchema = function (_ref) {
  var Joi = _ref.Joi;
  return Joi.object({
    ids: Joi.array().items(Joi.string()).default([]).description("Google Tag Manager IDs that can be found in your Tag Manager dashboard."),
    ...
    gtmAuth: Joi.string().description("Google Tag Manager environment auth string."),
    gtmPreview: Joi.string().description("Google Tag Manager environment preview name."),
    ...
    selfHostedOrigin: Joi.string().default("https://www.googletagmanager.com").description("The origin where GTM is hosted.")
  });
};