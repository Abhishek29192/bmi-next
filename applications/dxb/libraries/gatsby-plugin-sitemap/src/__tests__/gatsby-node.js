...
describe(`gatsby-plugin-sitemap Node API`, () => {
  ...
  it(`should ignore path prefix for sitemap public path when creating creating index sitemap with ignoreSitemapPathPrefix`, async () => {
    const graphql = jest.fn();
    graphql.mockResolvedValue({
      data: {
        site: {
    ...
    expect(publicBasePath).not.toEqual(expect.stringContaining(assetPrefix));
  });
