...
describe("useShowWarningModal hook", () => {
  it("should not block 'beforeunload' event if formState==='Initialized'", () => {
    render({ data: { formStatus: FormStatus.Initialized } });
  ...
  it("should not block 'beforeunload' event if formState==='Edited'", () => {
    render({
      data: { formStatus: FormStatus.Edited }
    });
  ...
  it("removes 'beforeunload' event listener on unmount", () => {
    ...
    const { unmount } = render({ data: { formStatus: FormStatus.Edited } });
  ...
  it("should not block navigation if formState==='Initialized'", () => {
    const { history } = render({
      data: { formStatus: FormStatus.Initialized }
    });
  ...
  it("should not block navigation if formState==='Edited'", () => {
  ...
  it("removes blocked location and clears local storage after closing the dialog", () => {
    const initialRoute = "/initial-route";
    const { result } = render({
      data: { formStatus: FormStatus.Edited },
      route: initialRoute
