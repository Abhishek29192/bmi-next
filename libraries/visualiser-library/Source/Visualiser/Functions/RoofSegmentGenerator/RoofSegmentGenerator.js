...
export default (boundsBox2, tileMesh, tileInfo, tileMaterial) => {
	...
	var primaryTilesMesh = new THREE.InstancedMesh(tileMesh.geometry.clone(), tileMaterial, instanceCount);
	...
	var endOfRowTile = tileSlice(tileMesh.geometry, tileMesh.geometry.boundingBox.max.x - endOfRowTileWidth, 'x', 'right'); // Discard the left side
	var endOfPrimaryRowTileMesh = new THREE.InstancedMesh(endOfRowTile, tileMaterial, rowCount);
	...
	if(horizontalOffset != 0){
		...
		var endOfSecondaryRowTile = tileSlice(tileMesh.geometry, tileMesh.geometry.boundingBox.max.x - offsetRowEndTileWidth, 'x', 'right'); // Discard the left side
		var endOfSecondaryRowTileMesh = new THREE.InstancedMesh(endOfSecondaryRowTile, tileMaterial, rowCount);
	
		var startOfSecondaryRowTile = tileSlice(tileMesh.geometry, tileMesh.geometry.boundingBox.min.x + horizontalOffset, 'x', 'left'); // Discard the right side
		var startOfSecondaryRowTileMesh = new THREE.InstancedMesh(startOfSecondaryRowTile, tileMaterial, rowCount);
	}
	...
	var overlapRotation = verticalOverlap ? Math.atan2(tileThickness, (1 - verticalOverlap) * tileHeight) : 0;
	...
	for(var v = 0; v<rowCount; v++){
		...
		for(var h=0;h<tilesOnThisRow;h++){
			...
			placementHelper.position.set(rowX - tileBounds.min.x, 0, rowZ - tileBounds.min.z);
		...
		var amountSlicedOff = (v&1 && horizontalOffset) ? (tileWidth - offsetRowEndTileWidth) : (tileWidth - endOfRowTileWidth);
		
		placementHelper.position.set(rowX - tileBounds.min.x - amountSlicedOff, 0, rowZ - tileBounds.min.z);
		placementHelper.rotation.z = zRotation;
		placementHelper.rotation.x = overlapRotation;
		placementHelper.updateMatrix();
		
		if(v&1 && horizontalOffset){
			endOfSecondaryRowTileMesh.setMatrixAt( endOfSecondaryRowIndex++, placementHelper.matrix );
			
			// Secondary row also has a tile at the start too:
			placementHelper.position.set(boundsBox2.min.x - tileBounds.min.x, 0, rowZ - tileBounds.min.z);
			placementHelper.rotation.z = zRotation;
			placementHelper.rotation.x = overlapRotation;
			placementHelper.updateMatrix();
			startOfSecondaryRowTileMesh.setMatrixAt( startOfSecondaryRowIndex++, placementHelper.matrix );
			
		}else{
			endOfPrimaryRowTileMesh.setMatrixAt( endOfRowIndex++, placementHelper.matrix );
		}
