...
export default (bufferGeometry, axisInTileUnits, axis, keepSide) => {
	...
	var addIntersect = (vert1Index, vert2Index) => {
		...
		if(uvs){
			...
			newUvs.push(
				uvs.array[vert1Index*2],
				uvs.array[(vert1Index*2)+1]
			);
		}
		...
		if(aOnLeft && bOnLeft && cOnLeft){
			...
			if((aOnLeft && bOnLeft) || (!aOnLeft && !bOnLeft)){
				...
				if((rightIsKept && aOnLeft) || (!aOnLeft && !rightIsKept)){
					...
					// generate 2 tris:
					newTriangles.push(
						aIndex, bIndex, bcIntersect,
						aIndex, bcIntersect, caIntersect
					);
				}
			...
			// Check if b->c is the line that doesn't intersect
				...
				if((rightIsKept && bOnLeft) || (!bOnLeft && !rightIsKept)){
					...
					// generate 2 tris:
					newTriangles.push(
						bIndex, cIndex, caIntersect,
						bIndex, caIntersect, abIntersect
					);
				}
				...
				if((rightIsKept && cOnLeft) || (!cOnLeft && !rightIsKept)){
					...
					// generate 2 tris:
					newTriangles.push(
						cIndex, aIndex, abIntersect,
						cIndex, abIntersect, bcIntersect
					);
				}
	...
	if(newVerts.length){
		...
		slicedTile.setAttribute('position', new THREE.BufferAttribute(vertBuffer, 3));
		...
		if(normals){
			...
			slicedTile.setAttribute('normal', new THREE.BufferAttribute(normalBuffer, 3));
		}
	...
	slicedTile.index = new THREE.BufferAttribute( new Uint16Array(newTriangles), 1 );
