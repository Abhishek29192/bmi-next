...
class OrbitControls extends EventDispatcher {
  ...
  panUp(distance: number, objectMatrix: THREE.Matrix4) {
    if (this.screenSpacePanning === true) {
      ...
      this.panUpVector.crossVectors(this.camera.up, this.panUpVector);
    }
  ...
  pan(deltaX: number, deltaY: number) {
    ...
    if (this.camera.type === "PerspectiveCamera") {
      // perspective
      const position = this.camera.position;
      this.panFuncOffset.copy(position).sub(this.target);
      let targetDistance = this.panFuncOffset.length();
