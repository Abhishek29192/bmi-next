...
module.exports.up = (migration) => {
  ...
  homePage.createField('title')
    .name('Title')
    .type('Symbol')
    .required(true)
    .validations([{ unique: true }])

  homePage.createField('slides')
    .name('Slides')
    .type('Array')
    .required(true)
    .items({ type: 'Link', validations: [{ linkContentType: ['contactUsPage', 'promo', 'page'] }], linkType: 'Entry' })

  homePage.createField('overlapCards')
    .name('Overlap Cards')
    .type('Array')
    .validations([{ size: { min: 2, max: 4 } }])
    .items({ type: 'Link', validations: [{ linkContentType: ['contactUsPage', 'page'] }], linkType: 'Entry' })

  homePage.createField('sections')
    .name('Sections')
    .type('Array')
    .items({ type: 'Link', validations: [{ linkContentType: ['carouselSection'] }], linkType: 'Entry' })

  homePage.createField('showSignUpBanner')
