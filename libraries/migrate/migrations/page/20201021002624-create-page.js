...
module.exports.up = (migration) => {
  ...
  page.createField('sections')
    .name('Sections')
    .type('Array')
    .items({ type: 'Link', validations: [{ linkContentType: ['cardCollectionSection', 'form', 'leadBlockSection', 'pageLinksSection', 'tabsOrAccordionSection', 'twoColumnSection', 'villainSection'] }], linkType: 'Entry' })

  page.createField('exploreBar')
    .name('Explore Bar')
    .type('Link')
    .validations([{ linkContentType: ['navigation'] }])
    .linkType('Entry')

  page.createField('showSignUpBanner')
    .name('Show Sign Up Banner')
    .type('Boolean')

  page.changeFieldControl('title', 'builtin', 'singleLine')
  page.changeFieldControl('slug', 'builtin', '4CKq0CMT3K3rfsDpKSxhdn', { helpText: 'This will define the URL of the page. The page will be created at "https://www.bmigroup.com/{country-code}/{slug}".' })
