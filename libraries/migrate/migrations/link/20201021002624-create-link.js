module.exports.description = 'Create content model for Link'
...
module.exports.up = (migration) => {
  ...
  link.createField('label')
    .name('Label')
    .type('Symbol')
    .required(true)

  link.createField('linkedPage')
    .name('Linked Page')
    .type('Link')
    .validations([{ linkContentType: ['contactUsPage', 'page', 'teamPage'] }])
    .linkType('Entry')

  link.createField('icon')
    .name('Icon')
    .type('Symbol')
    .validations([{ regexp: { pattern: '' } }, { in: ['Facebook', 'Twitter', 'LinkedIn', 'User', 'BMI', 'Phone', 'Mail', 'YouTube', 'Icopal', 'Monier', 'Monarplan', 'Arrow', 'Zanda', 'AeroDek'] }])

  link.createField('isLabelHidden')
...
module.exports.down = migration => migration.deleteContentType('link')
